...
func certProviderByCertMode(mode, dir, hostname, eabKID, eabKey, email string) (certProvider, error) {
	...
	case "alpn":
		// ALPN-01 challenge mode
		// This mode uses ACME TLS-ALPN-01 challenge for certificate validation
		// It requires an ACME client configuration
		return NewALPNCertProviderFromFlags(dir, hostname, eabKID, eabKey, email)
	default:
...
// NewALPNCertProviderFromFlags creates an ALPN-01 cert provider from command-line flags.
func NewALPNCertProviderFromFlags(certdir, hostname, eabKID, eabKey, email string) (certProvider, error) {
	if hostname == "" {
		return nil, errors.New("hostname is required for ALPN-01 challenge")
	}
	if email == "" {
		return nil, errors.New("email is required for ALPN-01 challenge")
	}
	...
	// If using GCP ACME directory
	if eabKID != "" && eabKey != "" {
		keyBytes, err := decodeEABKey(eabKey)
		if err != nil {
			return nil, err
		}
		acmeClient.DirectoryURL = "https://dv.acme-v02.api.pki.goog/directory"
		// Note: ExternalAccountBinding is not directly available in acme.Client
		// It would need to be set during account registration
		_ = keyBytes // Placeholder for future implementation
	}
	
	// For ALPN-01, we need the key authorization
	// This would typically be obtained from the ACME server during the challenge
	// For now, we'll create a placeholder that will be used during the challenge
	keyAuth := "" // Will be set during the challenge
	
	return NewALPNCertProvider(acmeClient, hostname, keyAuth)
}
